...
def get_details_from(familyIdList, memberIdList):
    try:
        ...
        with spnDB.snapshot() as snapshot:
            query = 'with Query_1 as (SELECT fmm.family_id ,fmm.member_id, fmm.member_name, fmm.gender, fmm.member_local_name, fmm.birth_date, fmm.unique_health_id FROM family_member_master fmm where family_id in unnest(@familyIdList) and member_id in unnest(@memberIdList)), Query_2 as(SELECT family_id,member_id,MIN(screening_id) as screening_id, MIN(TO_JSON_STRING(outcome)) as outcome, MAX(FORMAT_TIMESTAMP("%Y-%m-%d %H:%M:%S%z", last_update_date, "Asia/Calcutta")) AS last_update_date FROM health_screening where family_id in unnest(@familyIdList) and member_id in unnest(@memberIdList) and concat(member_id, FORMAT_TIMESTAMP("%Y-%m-%d %H:%M:%S%z", TIMESTAMP(JSON_VALUE(update_register,"$[0].timestamp")), "Asia/Calcutta") ) in (select concat(member_id, FORMAT_TIMESTAMP("%Y-%m-%d %H:%M:%S%z", max_time, "Asia/Calcutta") ) from (select member_id,max(TIMESTAMP(JSON_VALUE(update_register,"$[0].timestamp"))) as max_time  from health_screening where family_id  in  unnest(@familyIdList) and member_id in unnest(@memberIdList) group by 1)) group by 1,2) select  Q1.member_id, Q1.member_name, Q1.gender, Q1.member_local_name, Q1.birth_date, Q1.unique_health_id, Q1.family_id, Q2.last_update_date, PARSE_JSON(Q2.outcome)as outcome from Query_1 Q1 left join Query_2 Q2 on Q1.family_id= Q2.family_id and Q1.member_id = Q2.member_id'
...
def search_by_unique_health_id(unique_health_id):
    try:
        ...
        with spnDB.snapshot() as snapshot:   
            ...
            query = 'SELECT fmm.family_id ,fmm.member_id, fmm.member_name, fmm.gender, fmm.member_local_name, fmm.birth_date, fmm.unique_health_id,null as last_update_date,null as outcome  from family_member_master fmm WHERE fmm.unique_health_id=@unique_health_id'
            results = snapshot.execute_sql(
                query,
                params= {
...
def search_by_mobile_number(mobile_number):
    try:
        ...
        with spnDB.snapshot() as snapshot:   
            ...
            # query = 'SELECT fmm.family_id,fmm.member_id from family_member_master fmm WHERE fmm.mobile_number=@mobile_number'
            # Below query is formulated as per discussion with Dr.V & Kiran for removing the outcome & last update date value. 26 May 2022 = b/233998552
            query = 'SELECT fmm.family_id ,fmm.member_id, fmm.member_name, fmm.gender, fmm.member_local_name, fmm.birth_date, fmm.unique_health_id,null as last_update_date,null as outcome  from family_member_master fmm WHERE  fmm.mobile_number=@mobile_number and fmm.mobile_number != 0'
            results = snapshot.execute_sql(
                query,
                params= {
...
def search_by_smart_card_id(pds_smartcard_id):
    try:
        ...
        with spnDB.snapshot() as snapshot:
            ...
            # Below query is formulated as per discussion with Dr.V & Kiran for removing the outcome & last update date value. 26 May 2022 = b/233998552
            query = 'SELECT fmm.family_id ,fmm.member_id, fmm.member_name, fmm.gender, fmm.member_local_name, fmm.birth_date, fmm.unique_health_id,null as last_update_date,null as outcome from family_member_master fmm WHERE  family_id in (select family_id from family_master fmly WHERE  fmly.pds_smart_card_id not in (333000000000,334000000000,0) and fmly.pds_smart_card_id=@pds_smartcard_id);'
            results = snapshot.execute_sql(
                query,
                params= {
...
def search_by_name(member_name, district_id, block_id, village_id, offset):
    try:
        ...
        # Below query is formulated as per discussion with Dr.V & Kiran for removing the outcome & last update date value. 26 May 2022 = b/233998552
        query = 'SELECT  fmm.family_id ,fmm.member_id, fmm.member_name, fmm.gender, fmm.member_local_name, fmm.birth_date, fmm.unique_health_id,null as last_update_date,null as outcome from family_member_master fmm WHERE fmm.district_id=@district_id'
        
        if block_id is not None and block_id != '':
            query += " and fmm.block_id=@block_id"

        if village_id is not None and village_id != '':
            query += " and fmm.village_id=@village_id"

        # query += ' and STARTS_WITH (LOWER(fmm.member_name),lower(@member_name)' Since we also need to search by last name as well. 
        
        query += ' and lower(fmm.member_name) LIKE lower(@member_name) order by fmm.member_name limit 3000 offset @offset'
