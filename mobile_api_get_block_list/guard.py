...
def validate_id_attribute(userId, districtId):
    """
    This method will validate the user id and district id type and format
    Input args: ids (user_id and district_id)
    Return: Boolean
    """
    is_valid_id = False
    try:
        if userId == "" and districtId == "":
            # cloud_logger.critical("Both userId and districtId are empty")
            print('Both userId and districtId are empty')
            return is_valid_id
        else:
...
def user_token_validation(userId, mobile):
    ...
    try:
        ...
        print("get_block_list user_token_validation ProgrammingError",e)  
        conn.rollback()
        return False
    except psycopg2.InterfaceError as e:
        print("get_block_list user_token_validation InterfaceError",e)
        reconnectToDB()
        return False
