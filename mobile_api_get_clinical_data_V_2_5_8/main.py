...
@app.route('/mobile_api_get_clinical_data', methods=['POST'])
                ...
                if not is_token_valid:
                    ...
                    if(lastUpdate!=''):
                        with spnDB.snapshot() as snapshot:   
                            query = "SELECT table_name from master_change_log where last_update_date>@lastUpdate" 
    ...
    try:
        ...
        for table in tables:
            if(table=="health_vaccination_master"):
                vaccines=[]
                with spnDB.snapshot() as snapshot:   
                    query = "SELECT vaccination_id, vaccination_name FROM health_vaccination_master where last_update_date>@lastUpdate" 
                    results = snapshot.execute_sql(
                        query,
                        params={"lastUpdate": lastUpdateTS},
                ...
                with spnDB.snapshot() as snapshot:   
                    query = "SELECT protocol_id, protocol_desc, protocol_value_high, protocol_value_low, protocol_type, protocol_inference FROM health_protocol_master where last_update_date>@lastUpdate" 
                    results = snapshot.execute_sql(
                                        query,
                                        params={"lastUpdate": lastUpdateTS},
                                        param_types={"lastUpdate": spanner.param_types.TIMESTAMP},                    
                ...
                with spnDB.snapshot() as snapshot:   
                    query = "SELECT  drug_id, drug_name, drug_type, dosage from health_drugs_master where drug_usage_type!=@drugType and last_update_date>@lastUpdate" 
                    results = snapshot.execute_sql(
                        query,
                        params={
                ...
                with spnDB.snapshot() as snapshot:   
                    query = "SELECT lab_test_id, lab_test_name, specimen_type, specimen_type_id, result_entry_type, facility_level from health_lab_tests_master where last_update_date>@lastUpdate" 
                    results = snapshot.execute_sql(
                        query,
                        params={"lastUpdate": lastUpdateTS},
                        param_types={"lastUpdate": spanner.param_types.TIMESTAMP},                    
                ...
                with spnDB.snapshot() as snapshot:   
                    query = "SELECT diagnosis_id, reference_id, diagnosis_name, service_name, service_id, revisit_days, duration_of_illness, default_drug_id from health_diagnosis_master where last_update_date>@lastUpdate" 
                    results = snapshot.execute_sql(
                                    query,
                                    params={"lastUpdate": lastUpdateTS},
                                    param_types={"lastUpdate": spanner.param_types.TIMESTAMP},                    
...
def fetch_token_userId(userId, req_token):
    ...
    try:
        query = "SELECT auth_token FROM user_master WHERE user_id=@userId"
        valid = False
        with spnDB.snapshot() as snapshot:
