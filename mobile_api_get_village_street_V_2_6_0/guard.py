    ...
    try:
        ...
        for id in ids:
            if id != None and isinstance(id, str) and id != "" and id != " ":
                ...
                print("ID is empty or not valid %s", str(id))
                # cloud_logger.critical("ID is empty or not valid %s", str(id))
                valid_ids.append(False)
...
def user_token_validation(userId, mobile):
    ...
    try:
        ...
        print("Village_street user_token_validation ProgrammingError",e)  
        conn.rollback()
        return False
    except psycopg2.InterfaceError as e:
        print("Village_street user_token_validation InterfaceError",e)
        reconnectToDB()
        return False
