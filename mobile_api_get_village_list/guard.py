...
def validate_id_attribute(userId, blockId):
    """
    This method will validate the user id and district id type and format
    Input args: ids (userId and blockId)
    Return: Boolean
    """
    is_valid_id = False
    try:
        if userId == "" and blockId == "":
            print("Both userId and blockId are empty")
            # cloud_logger.critical("Both userId and blockId are empty")
            return is_valid_id
        else:
            return validate_id(userId, blockId)
    except Exception as error:
        # cloud_logger.error("Error validating Id attribute : %s | %s | %s ", str(error), current_userId, current_appversion)
...
def user_token_validation(userId, mobile):
    ...
    try:
        ...
        print("get_village_list_from_block user_token_validation ProgrammingError",e)  
        conn.rollback()
        return False
    except psycopg2.InterfaceError as e:
        print("get_village_list_from_block user_token_validation InterfaceError",e)
        reconnectToDB()
        return False
