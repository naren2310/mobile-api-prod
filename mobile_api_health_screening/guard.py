...
def user_token_validation(userId, mobile, user_facility_id):
    ...
    spnDB_userId = 0
    try:
        query = "select user_id from user_master where mobile_number=@mobile and user_id=@user_id and facility_id=@facility_id"
        ...
        if (spnDB_userId != 0):         #Condition to validate userId exist in spannerDB
            ...
            cloud_logger.info("Unregistered User/Inputs mismatch.")            
            return False
    except Exception as error:
